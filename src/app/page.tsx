  ...
  useEffect(() => {
    ...
    const checkForBlankScreen = () => {
      ...
      const timer = setTimeout(() => {
        ...
        // If we're on / but see blank gradient (only navigation visible), refresh
        if (window.location.pathname === '/' && !hasMainContent) {
          const nav = document.querySelector('nav, header');
          const hasOnlyNav = nav !== null && document.body.children.length <= 2;

          if (hasOnlyNav) {
            console.log('🔄 Blank gradient detected on /, refreshing page');
